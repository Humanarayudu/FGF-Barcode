...
function generateCard() {
  ...
  function calcCode128Height(charCount) {
    // For very short inputs (2-4 chars), use much smaller height
    if (charCount <= 4) {
      return 50;
    }
    // For medium inputs (5-10 chars)
  ...
  const userHeight = calcCode128Height(username.length);
  const passHeight = calcCode128Height(password.length);
